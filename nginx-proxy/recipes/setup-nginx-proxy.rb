...
node[:deploy].each do |application, deploy|
    ...
    bash "nginx-proxy-run" do
        user "root"
        code <<-EOH
            docker run -d -p 80:80 -p 443:443 --restart=always --volumes-from #{deploy[:application]} --name nginx-proxy -v /built-app/programs/web.browser -v /etc/nginx/vhost.d -v /var/run/docker.sock:/tmp/docker.sock:ro jwilder/nginx-proxy
        EOH
    end
end
