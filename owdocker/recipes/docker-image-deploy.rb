...
node[:deploy].each do |application, deploy|
  ...
  bash "docker-run" do
    user "root"
    code <<-EOH
      docker run #{dockerenvs} --restart=always -h #{hostname} -v /built_app/programs/web.browser -v /etc/nginx/certs -v /etc/nginx/vhost.d -p #{node[:opsworks][:instance][:private_ip]}:8080:80 --name #{deploy[:application]} -d #{deploy[:environment_variables][:registry_image]}:#{deploy[:environment_variables][:registry_tag]}
    EOH
  end
